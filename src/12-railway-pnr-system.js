...
export function processRailwayPNR(pnrData) {
  ...
  const trainDetails = `Train: ${trainNumber} - ${trainName} | ${source} → ${destination} | Class: ${pnrData.classBooked}`;

  const passengerList = pnrData.passengers.map((person) => {
    let label;
    if (person.current === "CAN") label = "CANCELLED";
    else if (person.current.startsWith("WL")) label = "WAITING";
    else if (person.current.startsWith("RAC")) label = "RAC";
    else if (person.current.startsWith("B") || person.current.startsWith("S"))
      label = "CONFIRMED";
    else label = "CONFIRMED";

    return {
      formattedName:
        person.name.padEnd(20) + `(${person.age}/${person.gender})`,
      bookingStatus: person.booking,
      currentStatus: person.current,
      statusLabel: label,
      isConfirmed: label === "CONFIRMED",
    };
  });

  const confirmedCount = passengerList.filter(
    (x) => x.statusLabel === "CONFIRMED",
  ).length;
  const waitingCount = passengerList.filter(
    (x) => x.statusLabel === "WAITING",
  ).length;
  const cancelledCount = passengerList.filter(
    (x) => x.statusLabel === "CANCELLED",
  ).length;
  const racCount = passengerList.filter((x) => x.statusLabel === "RAC").length;
  ...
  const stats = {
    ...
    rac: racCount,
    allConfirmed: passengerList.every((x) => x.isConfirmed),
    anyWaiting: passengerList.some((x) => x.statusLabel === "WAITING"),
  };

  const activePassengers = passengerList.filter(
    (x) => x.statusLabel !== "CANCELLED",
  );
  const isChartPrepared = activePassengers.every((x) => x.isConfirmed);

  return {
    pnrFormatted: formattedPNR,
