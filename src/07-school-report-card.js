...
export function generateReportCard(student) {
  ...
  if (!student || typeof student !== "object" || typeof student.name !== "string" || student.name.trim() === "" || !student.marks || typeof student.marks !== "object") {
    return null;
  }

  const subjectsKeys = Object.keys(student.marks);
  ...
  const totalMarks = marksValues.reduce((sum, val) => sum + val, 0);

  const subjectCount = subjectsKeys.length;
